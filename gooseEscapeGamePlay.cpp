...
void fight(Actor & monster, Actor & player, int gameBoard[MAX_BOARD_Y][MAX_BOARD_X])
{
    ...
    if (key == TK_F)
    {
    	...
    	monster.restart_location("MAX"); // call restart locatino to reset monster at bottom right
    	...
    	else
		{
    		...
    		out.writeLine("The goose lost 1 health and is now at " + monster.get_health_string() + " health.");
		}
	}
}
...
void pickedUpWeapon(Actor const & player, int gameBoard[MAX_BOARD_Y][MAX_BOARD_X])
{ 
